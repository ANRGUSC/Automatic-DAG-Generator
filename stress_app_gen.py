...
def generate_scripts(dag,config_path,script_path,app_path,sample_path):
    ...
    for i in range(len(total_line)):
        ...
        f.write("\tcount = cpu_count()\n")
        f.write("\tcmd = 'stress --cpu %d --timeout %ds &' %(count,execution_time)\n")
        ...
        f.write("\t\tdest_info = line.strip().split(' ')[1:]\n")
    ...
    for i in range(1,N+1):
        ...
        file[i] = sample_path+'%sbotnet.ipsum'%(i)
        with open(file[i],'w') as sample:
            l = 'This is sample file %s: '%(str(t))
            sample.write(l)
...
def generate_stress_app(dummy_conf_file,dummy_app_path):
...
def generate_multiple_apps(dummy_app_root,dummy_conf_root,N,start,max_conf_num):
    for i in range(start,N+1):
        print(i)
        dummy_app_path= '%sdummyapp%d/'%(dummy_app_root,i)
        rand_config = random.randint(1,max_conf_num)
        dummy_conf_path = '%stask_config%d.yml'%(dummy_conf_root,rand_config)
        generate_stress_app(dummy_conf_path, dummy_app_path)

if __name__ == '__main__':
    ## generate multiple apps
    # dummy_app_root = 'dummy_app_list/'
    # if os.path.isdir(dummy_app_root):
    #   shutil.rmtree(dummy_app_root)
    # os.mkdir(dummy_app_root)
    ...
    #generate 1 app: python3 stress_app_gen.py --conf
    dummy_conf_file = 'task_config_cpu10_comp100.yml'
    dummy_app_path = 'stresscpuapp10/'
    generate_stress_app(dummy_conf_file,dummy_app_path)